...
/**
 * A (sub)formula containing the Lasted operator.
 * 
 * @author bianculli
 * 
 */
public class LastedFormula extends TrioFormula {

	// the time instant at which the formula is instantied
	private int ti;
...
//	real name of the variable; need for supporting symmetry
	...
	private String lvars_format = "\nbool flagLasted_%d=1;";

	private String extbc_format = "\ti=%d;\n\tdo\n\t\t::i<%d ->\n\t\t\tif\n\t\t\t\t::!(memo_%s[i]%s) -> flagLasted_%d=0;\n"
			+ "\t\t\t\t::(memo_%s[i]%s)->skip;\n\t\t\tfi;\n\t\t\ti++\n\t\t::i>=%d -> break;\n\tod;";
	
	private String bc_format="\tif\n\t\t::!(%s)->s=0; goto gen;\n\t\t::%s->skip;\n\tfi;\n";

	/***************************************************************************
	 * Builds a Lasted formula.
	 * 
	 * @param f
	 *            the child formula
	 * @param t
	 ...
	 * @param ti
	 *            time instant at which the formula is instantied
	 * @param id
	 *            id of the formula
	 * @param interval
	 ...
	 */
	public LastedFormula(TrioFormula f, TermFormula t, int ti, int id,
		...
		this.ti = ti;
		this.logicExpression = "flagLasted_" + id;
		...
		this.externalBodyCode = String.format(this.extbc_format, ti_prime,
				temporalConstant, this.varName, this.child_logicE, id, this.varName,
				this.child_logicE, temporalConstant);
