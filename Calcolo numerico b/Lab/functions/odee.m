function [t,y]=odee(ode,tspan,y0,options,varargin)
%   ODEE risolve un sistema di equazioni differenziale con il metodo di Eulero
%   [t,y] = odee(ode,tspan,y0,options) con tspan = [t0 tfinal] integra il
...
%   
%   [t,y] = odee(ode,tspan,y0,options,p1,p2) accetta in ingresso p1,p2,... parametri addizionali
...
%      utilizzando il metodo di Eulero con passo h=0.1. La funzione f(t,y) è definita come una
...
%        [t,Y]=odee(@F,[0 15],[0.5;0],h,R,L,C,Vs);
%        plot(Y(:,1),Y(:,2))
...
%
%   Vedere anche 
%       funzioni per l'integrazione di equazioni differenziali:  ODEH, ODERK4, ODE12
...
%
%   NOTE:
%     Questa parte mostra la sintassi di ODEE per la versione v5.
%
%   Esempio
%         h = odeset('InitialStep',0.1)
%         [t,y]=odee('f',[0 10],4,h);   
...
for t=tt(1:end-1)
    yy=y(:,end);
    y=[y,yy+h*feval(ode,t,yy,varargin{:})];  %accoda alle soluzioni al passo precedente la nuova
end
t=tt';
y=y';
